...
async def search_workflows(
    ...
    try:
        ...
        # Load category mappings if category filter is applied
        category_filenames = None
        if category != "all":
            ...
            if search_categories_file.exists():
                ...
                category_filenames = set()
                for item in search_data:
                    item_category = item.get('category') or 'Uncategorized'
                    if item_category == category:
                        category_filenames.add(item.get('filename'))
        ...
        workflows, total = db.search_workflows(
            ...
            limit=per_page if category == "all" else 10000,  # Get all if filtering by category
            offset=offset if category == "all" else 0,
        ...
        # Apply pagination for category filter
        if category != "all":
            total = len(workflow_summaries)
            start_idx = offset
            end_idx = offset + per_page
            workflow_summaries = workflow_summaries[start_idx:end_idx]
